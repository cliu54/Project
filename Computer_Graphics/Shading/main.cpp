...
double ray_sphere_intersection(const Vector3d &ray_origin, const Vector3d &ray_direction, int index, Vector3d &p, Vector3d &N)
{
   ...
   double root = -1;
    ...
    {
        double root1 = (- b +std::sqrt(delta))/(2.0 * a);
        double root2 = (- b -std::sqrt(delta))/(2.0 * a);
        if(root1 > root2)
        {
            root =root1;
        }
...
double ray_parallelogram_intersection(const Vector3d &ray_origin, const Vector3d &ray_direction, int index, Vector3d &p, Vector3d &N)
{
    ...
    Matrix3d pgram_ray_intersect;
    pgram_ray_intersect << -pgram_u(0) , -pgram_v(0), ray_direction(0),
            -pgram_u(1) ,-pgram_v(1), ray_direction(1),
            -pgram_u(2) , -pgram_v(2), ray_direction(2);

    Vector3d pgram_to_ray = pgram_origin - ray_origin;

    Vector3d result = pgram_ray_intersect.colPivHouseholderQr().solve(pgram_to_ray);
    double u = result(0), v = result(1), t = result(2);
...
bool is_light_visible(const Vector3d &ray_origin, const Vector3d &ray_direction, const Vector3d &light_position)
{
    ...
    Vector3d p,N;
    int nearst_ojb = find_nearest_object(ray_origin+(1e-4)*ray_direction.normalized(),light_position.normalized(),p,N);
...
Vector4d shoot_ray(const Vector3d &ray_origin, const Vector3d &ray_direction, int max_bounce)
{
    ...
    for (int i = 0; i < light_positions.size(); ++i)
    {
        ...
        // pass in function to check whether list is visable
        if(is_light_visible(p,N,light_position)==false) 
        {
            continue;
        }
        ...
        // based on the formular in the slide for specular shading
    
        const Vector4d specular=obj_specular_color*std::pow(std::max(h.dot(N),0.0),obj_specular_exponent);
    ...
    // use refl_color
    const Vector4d shadow(0,0,0,0);
    
    Vector3d v = (ray_origin - p).normalized();
    Vector3d refl_direction =2*N*N.dot(ray_direction)-ray_direction;
    Vector3d refl_position = p +(1e-6)*refl_direction;
    if(max_bounce >0)
    {
        reflection_color = shoot_ray(refl_position,refl_direction.normalized(),max_bounce-1);
        reflection_color = reflection_color.cwiseProduct(refl_color);
    }
...
void raytrace_scene()
{
    ...
    // according to law of sine provided in doc
