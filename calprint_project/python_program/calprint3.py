...
class Calprint:
    ...
    def get_events_for_day(self, dt):
        ...
        for event in events:
            if(event["DTSTART"].year == dt.year and event["DTSTART"].month == dt.month and event["DTSTART"].day == dt.day):
                ...
                body += "\n" +'{:>8} to {:>8}: {} [{}]'.format(format_DThour_min, format_ENDhour_min, event["SUMMARY"], event["LOCATION"])
    ...
    def extract(self):
        ...
        # when regex matches the rule it will return the values to its corresponding variables
                ...
                event["DTSTART"] = datetime.datetime(int(year), int(month), int(day),int(hour),int(minute),int(second))
...
def main():
    # these three lines (186 & 187 & 188) are used to test program
    dt = datetime.datetime(2019,6,19)
    cal = Calprint("prudence.ics")
    cal.get_events_for_day(dt)
